...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
   ...
   hwndMainWnd = CreateWindowEx(WS_EX_APPWINDOW,//WS_EX_TOPMOST,
                                TEXT("thisApp"), TEXT("Guy On Platform"),
...
void drawScene()
{
   ...
   if (initializeScene)
   {
      ...
      world_HIDE[world_camera] = TRUE; // hide camera
   ...
   // move forward
   if (keyDown[VK_UP]) // up arrow key
   {
      // copy the guy's location to temporary buffer
      copyLocFromTo(&world_TRANS[world_guy*TRSZ], &buffer_TRANS[0], 1);
      
      // reset guy rotation
      copyRotFromTo(&world_STATICTRANS[world_guy*TRSZ], &world_TRANS[world_guy*TRSZ], 1);
      copyUcRotFromTo(&world_STATICTRANS[world_guy*TRSZ], &world_TRANS[world_guy*TRSZ], 1);
      
      // reset guy location
      copyLocFromTo(&world_STATICTRANS[world_guy*TRSZ], &world_TRANS[world_guy*TRSZ], 1);
      
      // move the guy in the direction of the camera shot
      localMove(&world_TRANS[0], world_guy, 0, 0.62, 0);
      localRot(&world_TRANS[0], world_guy, 0, 0, 0, 0, 0, world_TRANS[world_camera*TRSZ+ROTZ]);
      
      // move the guy back where he was plus the additional forward movement
      localMove(&world_TRANS[0], world_guy, buffer_TRANS[LOCX], buffer_TRANS[LOCY], 0);
   }

   // move backward
   if (keyDown[VK_DOWN]) // down arrow key
   {
      // copy the guy's location to temporary buffer
      copyLocFromTo(&world_TRANS[world_guy*TRSZ], &buffer_TRANS[0], 1);
      ...
      // move the guy in the direction opposite of the camera shot
      localMove(&world_TRANS[0], world_guy, 0, 0.62, 0);
      localRot(&world_TRANS[0], world_guy, 0, 0, 0, 0, 0, world_TRANS[world_camera*TRSZ+ROTZ]+180);
      
      // move the guy back where he was plus the additional backward movement
      localMove(&world_TRANS[0], world_guy, buffer_TRANS[LOCX], buffer_TRANS[LOCY], 0);
   }


   // copy camera rotation to temporary buffer
   copyRotFromTo(&world_TRANS[world_camera*TRSZ], &buffer_TRANS[0], 1);

   // reset camera rotation
   copyRotFromTo(&world_STATICTRANS[world_camera*TRSZ], &world_TRANS[world_camera*TRSZ], 1);
   copyUcRotFromTo(&world_STATICTRANS[world_camera*TRSZ], &world_TRANS[world_camera*TRSZ], 1);

   // move the camera just above the guy
   copyLocFromTo(&world_TRANS[world_guy*TRSZ], &world_TRANS[world_camera*TRSZ], 1);
   localMove(&world_TRANS[0], world_camera, 0, 0, 5);
   
   // rotate the camera at an angle relative to the guy's new position
   localRot(&world_TRANS[0], world_camera, world_TRANS[world_guy*TRSZ+LOCX],
                                           world_TRANS[world_guy*TRSZ+LOCY],
                                           world_TRANS[world_guy*TRSZ+LOCZ], 40, 0, 0);

   // rotate the camera back to where it was relative to the guy's new position
   localRot(&world_TRANS[0], world_camera, world_TRANS[world_guy*TRSZ+LOCX],
                                           world_TRANS[world_guy*TRSZ+LOCY],
                                           world_TRANS[world_guy*TRSZ+LOCZ], 0, 0, buffer_TRANS[ROTZ]);

   // turn camera left
   if (keyDown[VK_LEFT]) // left arrow key
   {
      localRot(&world_TRANS[0], world_camera, world_TRANS[world_guy*TRSZ+LOCX],
                                              world_TRANS[world_guy*TRSZ+LOCY],
                                              world_TRANS[world_guy*TRSZ+LOCZ], 0, 0, 10);
   }
   
   // turn camera right
   if (keyDown[VK_RIGHT]) // right arrow key
   {
      localRevRot(&world_TRANS[0], world_camera, world_TRANS[world_guy*TRSZ+LOCX],
                                                 world_TRANS[world_guy*TRSZ+LOCY],
                                                 world_TRANS[world_guy*TRSZ+LOCZ], 0, 0, 10);
   }
   ...
   applyMeshViewRotLocSz(&world_POINT[0], &world_POINTDATSZ[0], 0, world_MESHCNT,
                         &world_TRANS[world_camera*TRSZ], camLenseSz, rI);
